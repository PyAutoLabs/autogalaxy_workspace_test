...
inside an autogalaxy ``FitInterferometer`` fit. This is the interferometer
counterpart of ``fit_imaging_sparse_operator.py`` in this directory, and the
autogalaxy counterpart of the same-named script in
``autolens_workspace_test/scripts/misc/jax_assertions/`` (which runs the check
through a lensing ``Tracer``).

A galaxy list whose linear objects are a MIXED list — a linear light profile
func list (``lp_linear.Sersic``) plus one or more ``Mapper`` objects — is
fitted twice:
...
inside the mask (`1e6` wavelengths ~ 0.2"); shorter baselines leave the galaxy
...
grid = ag.Grid2D.from_mask(mask=real_space_mask)
...
galaxy_simulate = ag.Galaxy(
    redshift=0.5,
    bulge=ag.lp.Sersic(centre=(0.0, 0.0), effective_radius=0.2),
    disk=ag.lp.Sersic(centre=(0.05, 0.05), effective_radius=0.4),
)

dataset_simulated = simulator.via_galaxies_from(galaxies=[galaxy_simulate], grid=grid)
...
dataset = ag.Interferometer(
    ...
    noise_map=ag.VisibilitiesNoiseMap(np.repeat(sigma, 2, axis=1)),
    uv_wavelengths=uv_wavelengths,
...
def pixelization(shape):
    return ag.Pixelization(
        mesh=ag.mesh.RectangularUniform(shape=shape),
...
galaxy_light = ag.Galaxy(
    redshift=0.5,
    bulge=ag.lp_linear.Sersic(centre=(0.0, 0.0), effective_radius=0.2),
)

"""
The positive-only (NNLS) solver is the production default, but on a fit this
small its active set clamps all but one reconstructed value to exactly `0.0`,
which would make the `reconstruction` assertion vacuous. The direct solve is
used so every reconstructed value is non-trivial; the curvature matrix and data
vector that feed it are solver-independent.
"""
settings = ag.Settings(use_positive_only_solver=False)


def assert_parity(name, galaxies, rtol_reconstruction, rtol_log_evidence):
    fit_mapping = ag.FitInterferometer(
        dataset=dataset,
        galaxies=galaxies,
        settings=settings,
    )

    fit_sparse_operator = ag.FitInterferometer(
...
`F + H` here has condition number ~1e6, so the solve is well conditioned and
the reconstruction and log evidence agree at round-off — measured 1.4e-15 and
2.4e-15 respectively, against the 1e-11 / 1e-12 gates set below.
"""
assert_parity(
    name="linear light + one mapper",
    galaxies=[
        galaxy_light,
        ag.Galaxy(redshift=0.5, pixelization=pixelization(shape=(3, 3))),
    ],
...
Two galaxies, each with its own pixelization, so the MAPPER-MAPPER off-diagonal
blocks are exercised too. There is no ray tracing in autogalaxy, so both
mappers tile the SAME real-space region and differ only in mesh resolution
(3 x 3 against 4 x 4) — the hardest case for the off-diagonal blocks and the
one furthest from the single-mapper code path #500 left untouched.

That also makes the fit degenerate by construction: `cond(F + H)` sits at the
float64 floor (~7e16), so `F + H` is numerically singular. `curvature_matrix`
and `data_vector` are unaffected and still agree at round-off (4.2e-16 /
1.2e-16, identical to the one-mapper block), but the solve that consumes them
amplifies that round-off by an arbitrary amount: measured 9.0e-6 for
`reconstruction` and 7.0e-9 for `log_evidence`.
...
assert_parity(
    ...
    galaxies=[
        galaxy_light,
        ag.Galaxy(redshift=0.5, pixelization=pixelization(shape=(3, 3))),
        ag.Galaxy(redshift=1.0, pixelization=pixelization(shape=(4, 4))),
    ],
